...
Both editable in Settings — the spec listed them as separate user-editable fields. Sized to fit Groq free tier's 6K TPM cap without forcing the model to drop relevant context for typical demo-length sessions.
...
### Chat history capped at last 6 turns per request

Without a cap, every chat turn grows the request size linearly. By turn 4-5, a single request would exceed Groq free-tier's 6K TPM cap and be rejected as "too large." Six turns ≈ 3 exchanges, enough conversational continuity for natural follow-ups while keeping request size constant.
...
- **Free-tier rate limits.** Groq's free tier caps GPT-OSS 120B at 6,000 tokens per minute. Heavy use (rapid suggestion clicks plus auto-refresh in the same minute) will hit this limit. The app handles 429s gracefully with a friendly "Groq rate limit hit. Try again in ~30s." message. Users on Groq Dev tier (300K TPM, 50x the free tier) won't see this issue.
...
- **No transcript summarization for long meetings.** The detailed-answer context is a rolling window. For meetings longer than ~20 minutes, chat answers may lose access to early-meeting context. A production version would summarize older transcript and prepend the summary to the recent window.
...
Without a cap, every chat turn grows the request size linearly. By turn 4-5, a single request would exceed Groq free-tier's 8K TPM cap and be rejected as "too large." Six turns ≈ 3 exchanges, enough conversational continuity for natural follow-ups while keeping request size constant.
...
7. Auto-refresh timer fires every 45s. It calls `flush()` on the recorder to end the current take early, ensuring the latest audio is transcribed before the next suggestions call.
...
13. `streamChat()` POSTs to `/api/chat` with the detailed prompt, the last 10,000 chars of transcript, the last 6 chat turns, and the suggestion's preview + detail_seed.
